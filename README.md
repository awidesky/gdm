...
- **Window backends**: GLFW, FreeGLUT, or none
- **OpenGL loaders**: GLAD, GLEW, or **glew-glad** (GLEW API backed by GLAD function loading)
- **Optional math**: GLM integration
- **Optional debugging**: [GLUtil](#glutil--opengl-utility-library) - shader/model/texture loading, path resolution, logging, and a full debug system
- **Smart resolution**: Searches system-installed packages first, downloads source via tarball if not found
- **GLAD generation**: Fetches and generates GLAD bindings at CMake time via `gen.glad.sh` - no manual codegen
- **Debug/release dual GLAD**: Separate debug/release GLAD objects (debug mode enables KHR_debug, release mode strips it)
- **`glew-glad` hybrid**: Uses GLAD for actual GL entrypoint loading while exposing the GLEW API - lets codebases written against GLEW adopt debug features without rewriting includes
...
int main() {
    glfwInit();
    GLFWwindow* win = glfwCreateWindow(800, 600, "Hello GDM", nullptr, nullptr);
...
- **CMake** >= 3.24
- **C++17** compiler
- **curl** (required for `gen.glad.sh` GLAD code generation)
- **OpenGL** runtime + development headers

## Usage

### Option A: FetchContent (from remote)
...
### Option B: add_subdirectory (vendored)
