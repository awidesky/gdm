...
#include <glad/gl.h>
    #if GDM_DEBUG
    namespace glutil::debug { void init(bool printInfo); } // debug function declaration
    ...
    static inline int glutil_gladLoadGL(GLADloadfunc load) {
        ...
        if(ret) glutil::debug::init(false); //TODO_think false good?
        return ret;
    }
    ...
    static inline int glutil_gladLoadGLUserPtr(GLADuserptrloadfunc load, void* userptr) {
        ...
        if(ret) glutil::debug::init(false);
        return ret;
    }
